...
BUILD_DIR=~/halium-luneos-7.1
RESULT_DIR=${BUILD_DIR}/results
CPU_CORES=6
BUILD_VERSION="`date +%Y%m%d`-${BUILD_NUMBER}"
BASE_ARCHIVE_NAME="halium-luneos-7.1-`date +%Y%m%d`-${BUILD_NUMBER}"
...
build_device() {
  ...
  mkdir -p ${BUILD_DIR}/out/host/linux-x86/framework/
  ...
  mka systemimage
  if [ $? != 0 ]; then
      echo "Build of Halium 7.1 for $MACHINE failed"
      exit 1
  fi
  ...
  cp ${OUTPUT_DIR}/obj/KERNEL_OBJ/arch/arm64/boot/Image ${OUTPUT_DIR}/kernel-parts-${BUILD_VERSION}/
